...
7. Use TypeScript. Every Worker needs a default export with fetch().
...
`,
    "/src/index.ts": `// ${projectName} — generated by DevForge (offline template)
interface Todo {
  id: number;
  title: string;
  done: boolean;
...
const parseBody = async (req: Request): Promise<Partial<Todo>> => {
